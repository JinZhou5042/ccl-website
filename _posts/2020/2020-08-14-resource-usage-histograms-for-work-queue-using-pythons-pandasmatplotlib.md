...
<blockquote>
   <p>
    ...
    is a framework to write and execute master-worker applications. A master process that can be written in python, perl, or C generates the tasks that then can be remotely executed by worker processes. You can learn more about Work Queue
    <a href="https://cctools.readthedocs.io/en/latest/work_queue/">
     here
    </a>
    .
...
<br/></pre>
  <p>
   The resources measured are also written to Work Queue's transaction log.
   <span>
   </span>
   This log can be enabled when declaring the master's queue:
  </p>
...
q<span style="color: #333333;">.</span>enable_monitoring()</pre>
  <p>
   This log is also generated by
   <a href="https://cctools.readthedocs.io/en/latest/makeflow/">
    Makeflow
   </a>
   when using Work Queue as a batch system (-Twq).
   <span>
   </span>
   The resource information per task appears as a json object in the transactions marked as
   <span style="font-family: courier;">
    DONE end-state exit-code resource-exhausted resources-measured
   </span>
...
fig<span style="color: #333333;">.</span>savefig(input_file <span style="color: #333333;">+</span> <span style="background-color: #fff0f0;">'.png'</span>) </pre>
  ...
  <div class="col-sm-12">
    {% include figure.liquid path="/assets/blog/2020/resource-usage-histograms-for-work-queue-using-pythons-pandasmatplotlib/tr-bw-1-deep.log.log-d4de5d5f20.png" title="" class="img-fluid rounded z-depth-1" zoomable=true %}
  </div>
</div> </pre>
  <pre style="line-height: 125%; margin: 0;"> </pre>
  <pre style="line-height: 125%; margin: 0;"> </pre>
