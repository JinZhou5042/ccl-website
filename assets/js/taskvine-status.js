...
// query data using fetch with JSON fix
fetch(API_URL)
  .then(function (response) {
    return response.text();
  })
  .then(function (text) {
    // Fix invalid JSON with escaped single quotes
  ...
  .catch(function (error) {
    ...
    wrapper
      ...
      .text("Unable to load TaskVine manager data. Please try again later.");
  });
...
// clear filters by clicking page title
...
d3.select("#reload").on("change", function () {
  var checked = reload.property("checked");
...
function build_filter() {
  var filter = 'type=="vine_manager"';

  var host = get_query_string_value("host");
  if (host) {
    filter += ' and like(name + ":" + port, "' + host + '")';
  }

  var owner = get_query_string_value("owner");
  if (owner) {
    filter += ' and like(owner, "' + owner + '")';
  }

  var project = get_query_string_value("project");
  if (project) {
...
function add_manager(manager) {
  var row = wrapper
    .append("div")
    .attr("class", "row manager")
    .attr("id", manager.name + ":" + manager.port);
  ...
  // Fix: properly parse major version number
  var majorVersion = parseInt(manager.version.split(".")[0]);
...
function create_table(manager, column) {
  ...
  var catalog_url =
    "https://catalog.cse.nd.edu/detail/" +
    manager.address +
    ":" +
    manager.port +
    ":" +
...
function create_graph(manager, column, w, h) {
  ...
  // Add X axis
  var x = d3.scaleLog().clamp(true).domain([0.1, 1e6]).range([0, width]);
  ...
  // Add Y axis
  var y = d3.scaleBand().domain(groups).range([height, 0]);
  ...
  svg
    ...
    .attr("y", function (d) {
      return y(d.data.group) + rect_height / 2 - 3;
    })
    ...
    .on("mousemove", function () {
      var mouse = d3.mouse(wrapper.node());
      var mouseX = mouse[0] - 30;
      var mouseY = mouse[1] - 60;
...
function create_task_status_bar(manager, column) {
  ...
  svg
    ...
    .on("mouseover", function (d) {
      var content =
        d.category + ": " + parseInt(d.true_val, 10).toLocaleString();
...
function create_manager_time_pie(manager, column) {
  ...
  var data = [
    manager.time_send,
    manager.time_receive,
    manager.time_status_msgs,
    manager.time_internal,
    manager.time_polling,
    manager.time_application,
  ];
  var labels = [
    "Send Data",
    "Recv Data",
  ...
  svg = pieLegend.append("svg").attr("width", width).attr("height", height);
  var size = 15;
...
function create_worker_tasks_tabs(container, manager) {
  ...
  var target_id = "expandable-" + global_id;
  ...
  var worker_status_url = "https://" + host + "/worker_status";
  var task_status_url = "https://" + host + "/task_status";
      ...
      if (!cache[target_id].hasOwnProperty("worker")) {
        ...
        fetch(worker_status_url)
          ...
          .catch(function (error) {
            ...
            target.html("Unable to fetch worker data.");
          });
      } else {
      ...
      if (!cache[target_id].hasOwnProperty("task")) {
        ...
        fetch(task_status_url)
          ...
          .catch(function (error) {
            ...
            target.html("Unable to fetch task data.");
          });
      } else {
        show_task_data(target, cache[target_id].task, task_status_url);
      }
...
function show_task_data(container, data, refresh_url) {
    ...
    .on("click", function () {
      ...
      fetch(refresh_url)
        ...
        .catch(function (error) {
          ...
          container.html("Unable to refresh task data.");
        });
    });

  var activeTasks = data.filter(function (task) {
    return task.host != null;
  });
  var nonActiveTasks = data.filter(function (task) {
    return task.host == null;
  });

  var tasks = activeTasks.concat(nonActiveTasks);
  var i = 0;
  var page_size = 10;

  var displayData = tasks.slice(i, page_size);
  i += page_size;
  ...
  var tr = tbody.selectAll("tr").data(displayData).enter().append("tr");

  tr.selectAll("td")
    .data(function (d) {
      return [d.host, d.command, d.disk, d.memory, d.cores, d.gpus];
    })
    .enter()
    .append("td")
    .text(function (d) {
...
function show_worker_data(container, data, refresh_url) {
    ...
    .on("click", function () {
      ...
      fetch(refresh_url)
        ...
        .catch(function (error) {
          ...
          container.html("Unable to refresh worker data.");
        });
    });
  ...
  var header = ["host", "command", "resources"];
